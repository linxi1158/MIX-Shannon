...
class PairwiseHeader(nn.Module):
    ...
    def forward(self,
        ...
        loss, _, _ = self.loss_fn(feat, labels)
...
class ShuffleHeader(nn.Module):
    def __init__(self, csi_header: DictConfig, instruction_header: DictConfig, mix_instruction_csi_header: DictConfig):
        ...
        build_header: Callable = lambda cfg: nn.Sequential(OrderedDict(feat_proj=hydra.utils.instantiate(cfg.feat_proj),
                                                                       loss_fn=hydra.utils.instantiate(cfg.loss_cfg)))
    ...
    def forward(self, csi_info: Dict, instruction_info: Dict) -> Dict:
        ...
        # output
        output = {"csi_shuffle_loss": csi_loss,
    ...
    def calculate_mix_instruction_csi_loss(self,
                                           csi_feat: torch.Tensor,
                                           csi_label: List,
